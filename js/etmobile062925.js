$(function() { 
    ...
    var workHiRange = 87;
    var workNoteCount = 15;
    var displayNoteCount = 15;
    var workPauseCount = 1;
    var displayPauseCount = 1;    
    var workKey = 0;
    var displayKey = 0;
    var workScale = 0;
    var displayScale = 0;
    ...
    var instrumentNames = ['    Piano', '    Sax', '     Bass', ' Electric Clean', ' Acoustic Guitar'];
    var instrumentInfo = [{name: '    Piano', low: 0, high: 87}, 
                            {name: '    Sax', low: 19, high: 63},
                            {name: '     Bass', low: 7, high: 36},
                            {name: ' Electric Clean', low: 19, high: 63},
                            {name: ' Acoustic Guitar', low: 19, high: 63}];
    

    const arrNoteBuild = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B' ];
    var arrNoteNames = ['A0', 'A#0', 'B0'];  // first three then we can cycle through octaves from C to shining C
    ...
    $('button#keepButton').on('click', function() {
        ...
        displayLowRange = workLowRange;
        displayHiRange = workHiRange;
    ...
    $('button#keepButton2').on('click', function() {
        ...
        displayLowRange = instrumentInfo[displayInstrument].low;
        displayHiRange = instrumentInfo[displayInstrument].high;
        var elLowEnd = document.getElementById('lowRangeDisplay');
        elLowEnd.innerHTML = '<span>Range Low End: ' + arrNoteNames[displayLowRange].toString() + '</span>';
        $('#acLowRange').find('.redx').remove();
    ...
    function InstrumentLoad() {    
        ...
        Tone.Master.volume.value = (-25 + (workVolume * 3));             
    };
